 ...
 */
public class PoolConnectionFactory implements ConnectionFactory, ExceptionListener, InitializingBean, DisposableBean {
	 ...
	 */
	public PoolConnectionFactory(List<Connection> targetConnections) {
		Assert.notNull(targetConnections, "Target Connection must not be null");
		this.connectionPool = targetConnections;
	}

	/**
	 * Create a new PoolConnectionFactory that always returns a single Connection
	 * that it will lazily create via the given target ConnectionFactory.
	 * @param targetConnectionFactory the target ConnectionFactory
	 */
	public PoolConnectionFactory(ConnectionFactory targetConnectionFactory) {
	...
	public void afterPropertiesSet() {
		if (this.connectionPool == null && getTargetConnectionFactory() == null) {
			throw new IllegalArgumentException("Target Connections or ConnectionFactory is required");
		}
	}
	...
	protected Connection getConnection() throws JMSException {
		synchronized (this.connectionMonitor) {
			if (this.connectionPool == null || this.connectionPool.size() ==0) {
				initConnection();
			}
			return this.connectionPool.get(0);
		}
	}
	...
	public void initConnection() throws JMSException {
		...
		synchronized (this.connectionMonitor) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Established shared JMS Connection: " + connection);
			}
	...
	private class SharedConnectionInvocationHandler implements InvocationHandler {
		...
		private void localStart() throws JMSException {
			synchronized (connectionMonitor) {
				if (!this.locallyStarted) {
					...
					if (startedCount == 0 && connectionPool != null) {
		...
		private void localStop() throws JMSException {
			synchronized (connectionMonitor) {
				if (this.locallyStarted) {
					...
					if (startedCount == 1 && connectionPool != null) {
						connectionPool.forEach(connection -> {
							try {
								connection.stop();
							} catch (JMSException e) {
		...
		private PoolConnectionFactory factory() {
			return PoolConnectionFactory.this;
		}
	}
	...
	protected List<Connection> newPooledConnection(Connection connection) {
		this.connectionPool.add(connection);
	 ...
	 */
	protected List<Connection> createConnectionPool(List<Connection> connections) {
